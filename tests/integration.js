...
const createTmp = () => {
  return new Promise((resolve, reject) => {
    tmp.dir(
      { template: "test-next-aws-lambda-webpack-plugin-XXXXXX" },
      function _tempDirCreated(err, tmpDirPath, cleanupCallback) {
        if (err) reject(err);
  ...
  fs.copySync(localExamplePath, tmpDirPath, {
    filter: (entry) => {
      return !!path
        ...
        .match(
          /^..\/example\/?((pages|distribution|public|next\.config\.js).*)?$/g
        );
    },
  });
};
